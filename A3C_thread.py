...
def run_process(process_id, shared_weight_list, shared_rmsprop_params):
    global running_reward, accumulated_num_frames, shared_model
    env = gym.make("Pong-v0")
    gpu_on = args.gpu_on   # FIXME: still problems for gpu
    observation = env.reset()
    print "start_reward = %f" % running_reward
    if args.shared_opt:
    ...
    while True:
        ...
        if (done) or time_step_index >= t_max:
            ...
            if time.time() - last_checkpoint_time > 1000 and done and process_id == 0:  # save every 1000 seconds
                filename = 'excited_%d.pkl' % accumulated_num_frames
            ...
            if done:
                ...
                accumulated_num_frames += action_label_len
                print "process_id = %d, step #%d, reward_sum = %f, running_reward = %f, average_policy = %s, average_value = %s, num of frames = %d" % \
                        (process_id, accumulated_num_frames, reward_sum, running_reward, str(average_policy), str(average_value), action_label_len)
                ...
                print "time per M step = %f h" % ((time.time() - start_time ) * 1000000 / 3600 / accumulated_num_frames)
                # time.sleep(0.1)
...
if __name__ == '__main__':
    ...
    lock_1 = th.Lock()
    running_reward = args.start_reward
    accumulated_num_frames = args.start_step
    ...
    for _1, rms_optimizer in enumerate(shared_model.get_all_optimizers()):
        ...
        for item in rms_optimizer._states.keys():
            shared_rmsprop_params[_1][item] = rms_optimizer._states[item]['ms'].flatten()

    processes = [[]] * args.process_num
    for item in range(args.process_num):
        processes[item] = th.Thread(target=run_process, 
